...
from __fixtures__.fixture_helper import convert_to_std_object, load_json_fixture
...
def test_get_pypi_package_data__happy_case(mock_cve_request_json, mock_pypi_request_json):
    # Given
    mock_pypi_request_json.return_value = load_json_fixture("__fixtures__/pypi/pypi_org_pypi_aws-encryption-sdk_1_2_0.json")
    mock_cve_request_json.return_value = load_json_fixture("__fixtures__/cve/services_nvd_nist_gov_rest_json_cve_1_0_CVE_2013_5123.json")
    expected_output = {
        ...
        'vulnerabilities': [{
            ...
            'overview': '',
            'recommendation': 'Update package to non-vulnerable '
                              'version 2.0.0',
            'references': [],
            'severity': 'medium',
            'version': '',
    ...
    # Then
    output["vulnerabilities"] = list(map(convert_to_std_object, output["vulnerabilities"]))
